...
function siteorigin_panels_lite_localization( $key = false ){
	...
	if( empty($loc) ) {
		$loc = apply_filters( 'siteorigin_panels_lite_localization', array(
			'page_builder' => __( 'Page Builder', 'snapshot' ),
			'home_page_title' => __( 'Custom Home Page Builder', 'snapshot' ),
			'home_page_menu' => __( 'Home Page', 'snapshot' ),
			'install_plugin' => __( 'Install Page Builder Plugin', 'snapshot' ),
			'on_text' => __( 'On', 'snapshot' ),
			'off_text' => __( 'Off', 'snapshot' ),

			// Longer message to display to a user about installing the plugin
			'home_install_message' => __( 'This theme supports Page Builder to create beautifully proportioned column based content.', 'snapshot' ),
