	...
	function customize_register( $wp_customize ){
		...
		if( method_exists($wp_customize, 'add_panel') ) {
			$wp_customize->add_panel( 'page_settings', array(
				'title' => __( 'Page Template Settings', 'snapshot' ),
				'description' => __( 'Change layouts for various pages on your site.', 'snapshot' ),
				'priority' => 11,
			) );
		}
		...
		$types = array();
		$templates = array(
			'home' => __( 'Blog Page', 'snapshot' ),
			'search' => __( 'Search Results', 'snapshot' ),
			'date' => __( 'Date Archives', 'snapshot' ),
			'author' => __( 'Author Archives', 'snapshot' ),
			'404' => __( 'Not Found', 'snapshot' ),
		);
		...
		foreach( $post_types as $post_type => $post_type_data ) {
			...
			$types[] = array(
				'group' => 'archive',
				'id' => $post_type,
				'title' => __( 'Type', 'snapshot' ) . ': ' . $post_type_data->label
			);
		}
		...
		foreach( $taxonomies as $tax_slug => $taxonomy ) {
			...
			$types[] = array(
				'group' => 'taxonomy',
				'id' => $tax_slug,
				'title' => __( 'Taxonomy', 'snapshot' ) . ': ' . $taxonomy->label
			);
		}
