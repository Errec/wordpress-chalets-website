...
class SiteOrigin_Settings_About_Page {
	...
	function about_page_notice(){
		...
		<div class="updated notice is-dismissible">
			<p>
				<?php echo esc_html( sprintf( __( 'Thanks for choosing %s!', 'snapshot' ), $theme->get( 'Name' ) ) ); ?>
				<?php
				printf(
					esc_html__( 'You can learn more about it %shere%s, or head straight to the %scustomizer%s to start setting it up.', 'snapshot' ),
			...
			<p>
				<a href="<?php echo esc_url( admin_url( 'themes.php?page=siteorigin-theme-about' ) ); ?>" class="button-primary">
					<?php echo esc_html( sprintf( __( 'Learn About %s', 'snapshot' ), $theme->get( 'Name' ) ) ); ?>
				</a>
			</p>
		</div>
		<?php
	...
	function add_theme_page( ){
		...
		add_theme_page(
			sprintf( __( 'About %s', 'snapshot' ), $theme_name ),
			sprintf( __( 'About %s', 'snapshot' ), $theme_name ),
			'edit_theme_options',
			...
			case 'twitter' :
				$share_url = add_query_arg( array(
					'status' => urlencode(
						$theme->get( 'Name' ) .
						' - ' .
						__( 'Free #WordPress Theme from @SiteOrigin', 'snapshot' ) . ' - ' .
						$theme->get( 'ThemeURI' )
					)
	...
	function display_about_page(){
		...
		$about = apply_filters( 'siteorigin_about_page', array(
			'title' => sprintf( __( 'About %s', 'snapshot' ), $theme->get( 'Name' ) ),
			'sections' => array(),
			'title_image' => false,
		...
		<div class="wrap" id="siteorigin-about-page">
				...
				<?php if( !empty( $about[ 'tour_url' ] ) ) : ?>
					<li>
						<a href="<?php echo esc_url( $about[ 'tour_url' ] ) ?>" class="about-button-tour" target="_blank">
							<?php esc_html_e( 'Take a Tour', 'snapshot' ) ?>
						</a>
					</li>
				<?php endif; ?>
				...
				<?php if( !empty( $about[ 'newsletter_url' ] ) ) : ?>
					<li>
						<a href="<?php echo esc_url( $about[ 'newsletter_url' ] ) ?>" class="about-button-updates" target="_blank">
							...
							<?php esc_html_e( 'Get Updates', 'snapshot' ) ?>
						</a>
					</li>
				<?php endif; ?>
				...
				<?php if( !empty( $about[ 'review_url' ] ) ) : ?>
					<li>
						<a href="<?php echo esc_url( $about[ 'review_url' ] ) ?>" class="about-button-updates" target="_blank">
							...
							<?php esc_html_e( 'Write a Review', 'snapshot' ) ?>
						</a>
					</li>
				<?php endif; ?>
				...
				<?php if( !empty( $about[ 'premium_url' ] ) ) : ?>
					<li class="about-highlight">
						<a href="<?php echo esc_url( $about[ 'premium_url' ] ) ?>" class="about-button-updates" target="_blank">
							...
							<?php esc_html_e( 'Upgrade to Premium', 'snapshot' ) ?>
						</a>
					</li>
				<?php endif; ?>
					...
					<div class="about-container">
						...
						<div class="about-video-watch">
							<a href="<?php echo esc_url( $about[ 'video_url' ] ) ?>" target="_blank">
								<?php esc_html_e( 'Watch The Video', 'snapshot' ) ?>
							</a>
						</div>
						...
						<?php if( $theme->get( 'ThemeURI' ) ) : ?>
							<div class="about-share">
								<div class="about-share-title">
									<?php echo esc_html( sprintf( __( 'If you like %s, please share it!', 'snapshot' ), $theme->get( 'Name' ) ) ) ?>
								</div>
			...
			<div class="about-siteorigin-logo">
				<p>
					<?php echo esc_html( __( 'Proudly Created By', 'snapshot' ) ) ?>
				</p>
