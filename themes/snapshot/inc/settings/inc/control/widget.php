...
class SiteOrigin_Settings_Control_Widget extends WP_Customize_Control {
	...
	function render_content( ){
		...
		if( !class_exists( $this->widget_args['class'] ) ) {
			...
			_e('This field requires the Widgets Bundle plugin.', 'snapshot');
			echo ' ';
			printf( __( '<a href="%s">Install</a> the Widgets Bundle now.', 'snapshot' ), 'https://wordpress.org/plugins/so-widgets-bundle/' );
			?></div>
			...
			$form = '<p><a href="" class="button-secondary so-widget-close">' . __( 'Close', 'snapshot' ) . '</a></p>' . ob_get_clean();
			...
			$form .= '<p><a href="" class="button-secondary so-widget-close">' . __( 'Close', 'snapshot' ) . '</a></p>';
			...
			<div class="so-settings-widget-form">
				...
				<a href="#" class="button-primary so-edit-widget"><?php esc_html_e( 'Edit Widget', 'snapshot' ) ?></a>
			</div>
			<?php
		}
	}
